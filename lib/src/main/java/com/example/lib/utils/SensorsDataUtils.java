            ...
            if (TextUtils.isEmpty(userAgent)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    try {
                        ...
                        ReportLog.logD("WebSettings NoSuchMethod: getDefaultUserAgent");
                    }
                }
    ...
    public static boolean checkHasPermission(Context context, String permission) {
        try {
            ...
            if (result != PackageManager.PERMISSION_GRANTED) {
                ReportLog.logD( "You can fix this by adding the following to your AndroidManifest.xml file:\n"
            ...
            if (tm != null) {
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
                    if (tm.hasCarrierPrivileges()) {
                        ...
                        ReportLog.logD( "Can not get IMEI info.");
                    }
    ...
    private static boolean hasReadPhoneStatePermission(Context context) {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
            if (!SensorsDataUtils.checkHasPermission(context, Manifest.permission.READ_PRECISE_PHONE_STATE)) {
                ReportLog.logD("Don't have permission android.permission.READ_PRECISE_PHONE_STATE,getDeviceID failed");
                return false;
            }
        } else if (!SensorsDataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE)) {
            ReportLog.logD( "Don't have permission android.permission.READ_PHONE_STATE,getDeviceID failed");
            return false;
        }
        return true;
    }
